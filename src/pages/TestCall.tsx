        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className={`relative flex h-11 w-11 items-center justify-center rounded-xl transition-colors ${
              call.state === 'ringing' ? "bg-amber-100 text-amber-600" : "bg-brand-50 text-brand-600"
            }`}>
              {call.state === 'ringing' ? (
                <motion.div
          ...
          <div className="flex flex-col items-end gap-1">
             <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-slate-50 border border-slate-100">
                <div className={`h-1.5 w-1.5 rounded-full ${call.state === 'ending' ? 'bg-rose-500' : 'bg-emerald-500 animate-pulse'}`} />
              ...
              <div className="mb-1.5 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <div className={`h-5 w-5 rounded-md flex items-center justify-center ${
                    call.lastTranscript.speaker === "ai" ? "bg-slate-200 text-slate-600" : "bg-brand-200 text-brand-700"
                  }`}>
                ...
                <div className="flex gap-0.5">
                    <span className="w-1 h-1 rounded-full bg-brand-300 animate-bounce" style={{ animationDelay: '0ms' }} />
                    <span className="w-1 h-1 rounded-full bg-brand-300 animate-bounce" style={{ animationDelay: '150ms' }} />
                    <span className="w-1 h-1 rounded-full bg-brand-300 animate-bounce" style={{ animationDelay: '300ms' }} />
                </div>
              </div>
           ...
           {activeCalls.length > 0 && (
             ...
             >
                <div className="relative h-2 w-2">
                   <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75">{activeCalls.length}</span>
                </div>
                <span className="text-xs font-black text-emerald-700"> ACTIVE SESSIONS</span>
             </motion.div>
           )}
           
           <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl border ${
             sseConnected ? "bg-brand-50 border-brand-100 text-brand-700" : "bg-red-50 border-red-100 text-red-700"
           }`}>
